...
class Enemy{
    ...
    start(){
        ...
        this.frameY=(Math.floor(Math.random()*4))*this.height;
        if(Math.random()<.5){
            this.x=Math.random()*this.game.width;
            this.y= this.y=Math.random()<.5 ? -this.radius :this.game.height +this.radius;
            
        } else {
            this.x=Math.random()<.5 ? -this.radius :this.game.width +this.radius;
        ...
        this.angle= Math.atan2(aim[3],aim[2]) + Math.PI*.5 ;
    }
    reset(){
        this.free=true;
    }
    ...
    draw(context){
        if(!this.free){
            ...
            context.translate(this.x,this.y)
            context.rotate(this.angle);
            context.drawImage(this.image,this.frameX*this.width,this.frameY,this.width,this.height, -this.radius,-this.radius,this.width,this.height);
            if(this.game.debug){
                context.beginPath();
            context.arc(0, 0, this.radius, 0, Math.PI * 2);
            context.stroke();
...
class Beetlermorph extends Enemy {
    constructor(game){
        super(game);
...
class Game {
    ...
    render(context,deltaTime){
       ...
       if(this.gameOver) this.destroyAllEnemies();
    }
    destroyAllEnemies(){
        this.enemyPool.forEach(enemy => {
            if (!enemy.free) {
                enemy.reset(); // Set the enemy as "free" to remove them from the screen
            }
        });
    }
    checkCollision(a,b){
    ...
    drawStatusText(context){
        ...
        for(let i=0;i<this.lives;i++){
            context.fillRect(20 + 25 * i, 60, 10,30);
        ...
        if(this.gameOver){
            ...
            if(this.score>=this.winningScore) {
               ...
               context.fillText(message1,this.width*.5,170);
               context.font='50px Impact';
               context.fillText(message2,this.width*.5,500);
    ...
    createEnemyPool() {
        for (let i = 0; i < this.numberOfEnemies; i++) {
            ...
            this.enemyPool.push(new Beetlermorph(this));
        } else{
